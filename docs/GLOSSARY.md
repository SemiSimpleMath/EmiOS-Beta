...
The belief-relation agent (`belief_engine::merge_verifier`, `gpt-5.6-luna`) — the precision gate for both write-time dedup (Step 2, `dedup_candidate`) and the weekly sweep (Step 5, `canonicalize_belief_set.py`). Two recall channels (embedding NN ≥ 0.80, and shared-distinctive-keyword) *propose* candidate **pairs**; `merge_verifier` *decides* each pair's relation, seeing each side's area, kind, observation count, dates and recent dated evidence. Relations: `same` (merge), `different` and `specialises` (both stand, verdict recorded in `belief_distinct_pairs`), `supersedes` (the outdated side is deprecated), `contradicts` (both marked `contested` for the reevaluator). It is **asymmetric** — `same` is never the safe guess, because a wrong merge silently destroys a distinct belief — and an unreadable relation falls back to `different`, changing nothing. On `same`, the better-supported belief survives (its statement rewritten to the reconciled `canonical_statement`), the loser is deprecated (`merge_belief`), and the **belief_merges** redirect is recorded. Owner-**locked** beliefs are excluded. Replaced the dead chunk-based `belief_canonicalizer`. See [16_BELIEF_ENGINE.md](architecture/16_BELIEF_ENGINE.md) §4.
