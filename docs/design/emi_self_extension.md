# Emi extends Emi — self-modification capability

Status: draft for discussion. No code yet. Companion to `coding_agent_for_emi.md` and `cli_emi.md`.

## The goal

User: "Emi, I want to view my Ring camera feed in the UI."

Emi: ...edits her own code, registers a new blueprint, adds a template, calls the existing `ring_camera_control` tool, and surfaces the new `/ring` page in the running UI. Either auto-reloaded or after a restart, the page exists.

That's the golden ring. **Emi can extend Emi.** New UI pages, new agents, new managers, new tools — whatever the user asks for, Emi adds it to herself.
...
A coder editing some random Python project has it relatively easy: read the code, infer the patterns, edit, test, ship. A coder editing the very Emi process it's running inside has these additional constraints:

1. **Conventions are dense and project-specific.** Emi has a lot of structural patterns (room directories, agent directories, manager configs, tool contracts, control nodes, j2 templates, Pydantic forms). A generic LLM prompt won't know "to add a manager you need config.yaml with role_bindings + agents + control_nodes + flow_config + state_map; the manager is auto-discovered if its directory follows the convention." The coder either learns these conventions inline (by reading similar code) or gets them in its system prompt (rigid, can stale).

2. **The blast radius IS the system.** If a generic coder breaks tests in a workspace, the workspace is broken. If Emi's self-coder breaks `app/assistant/`, **the next user message can't be processed.** Flask either fails to import (hard down) or imports but raises at runtime when the modified path executes. Either way, recovery has to be possible without "ask Emi to fix it" because Emi can't talk anymore.
...
4. **Verification is harder.** A generic coder can run `pytest` and trust the green/red signal. An Emi self-coder needs to also verify "does the new feature actually appear in the running UI?" — a runtime question, not a static one. Smoke tests against a live `/ring` endpoint, screenshot the rendered page, etc.
...
What does "add a Ring viewer to Emi" actually decompose into? Walking it through end-to-end gives us the concrete requirement list.

### What the user wants
"Show me my Ring camera snapshots in the UI. Let me click a button to grab a fresh one."

### What that requires Emi to do
...
8. **Audit.** Every file edited goes into a `kg_revision_log`-equivalent for code changes (`code_revision_log`?). Reversible — running `git diff HEAD~1` shows exactly what Emi did to herself. If something feels wrong, `git revert HEAD` undoes it.

This is roughly 4-7 edits across 4-5 files, with one new dependency between the route and the existing pod_store. Maybe 200 lines of net-new code plus 10-line edits in two existing files. **Mechanically simple. The hard part is knowing Emi well enough to do it cleanly.**

### What needs to exist for Emi to do it autonomously
...
The right answer is **both**. System prompt has the high-level map ("Emi has rooms / managers / agents / tools / control_nodes; here's roughly where each lives; here's the registration story"). The agent's discipline is "read existing examples before writing new ones."
...
- **Smoke runtime.** Spawn a sub-process that imports Emi's app factory (or hits a `/health` endpoint if Flask is already running with hot reload). Check it doesn't crash on startup.
...
Self-modification breaking Emi is the worst-case scenario. Recovery strategy:
...
- **A watchdog process.** Out-of-band of the main Emi process: a small script that monitors Emi's health (`curl /health` every 10s). If Emi goes down after a coder commit, the watchdog can `git revert` the last commit and signal Emi to restart.
...
6. **Coder system prompt extensions** — Emi-specific discipline (find prior art, follow conventions, name files predictably, register where required). ~1 day, plus iteration.

**Self-extension delta on top of generic coder:** ~5-7 days. Generic coder is 7-9 days (with critic, per the latest estimate). **Total path from zero to "Emi extends Emi": ~12-16 days, or 2.5-3 weeks of focused work.**
...
3. Coder system prompt extended with Emi-specific discipline.
4. Auto-commit per turn.
5. Smoke verification harness (Flask health check, route reachability, registry lookup).

After Phase 2: ask the coder to add a small feature (a new resource template, a new dayflow item type, a new pod tag, etc.). Iterate on prompts and tier classifications. Use the Ring viewer as the milestone test — if Emi can add the Ring viewer end-to-end with one user prompt, Phase 2 is done.
...
After Phase 3: a self-modification that breaks Emi is automatically reverted within ~30 seconds. User can also manually rollback any coder commit.
...
- **Recursive self-improvement loops.** User: "Emi, make yourself smarter." Emi: edits its own LLM model selection / prompt strategies / tool catalog. This is the AI safety classic. Mitigation: Tier 3 forbids agent runtime / agent prompt edits. The coder cannot modify its own discipline.
...
- **The user inadvertently blesses bad changes.** "Yes, just add the page" — and the agent does, but the page has a security flaw. Mitigation: the critic LLM stage-B review catches obvious nonsense; the user's trust pattern matters. Document that "Emi's self-extension is helpful but not a substitute for code review."
...
6. **Does the coder participate in dayflow?** I.e., can a dayflow item be "Emi please add a Ring viewer when you have time"? Probably yes eventually, but not v1. v1 is foreground only.
...
If this works, the user experience changes meaningfully. Today: "I want a Ring viewer page" requires the user to understand Flask routes, templates, blueprints, the existing pod system, and write the code. With self-extension working: "I want a Ring viewer page" is one sentence to Emi, and 5-10 minutes later the page exists.
...
**My honest assessment of feasibility:** Phase 1 + 2 (generic coder + Emi awareness) is hard but achievable in 3 weeks. Phase 3 (watchdog + rollback) is operational work, also achievable. Whether the resulting system is *actually pleasant to use* depends entirely on the coder agent's prompt quality, which is iteration-driven and can't be predicted from design alone.

The Ring viewer is the right milestone. It's the simplest case study that exercises every piece — file edits, blueprint registration, template, static assets, integration with an existing tool, runtime verification. If Emi can do the Ring viewer end-to-end, she can do most small feature requests.
...
2. Decide whether this is something you want to chase. The answer might honestly be "no" if Claude Code already covers most of your needs and the maintenance burden of a self-extending Emi feels heavy.
...
4. The Ring viewer is the v1 milestone. If Emi can land it cleanly, the system is ready.
