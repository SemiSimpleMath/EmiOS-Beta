...
//      (Marriage, Family Time, Parent of Sam ...).
...
export function GraphCanvas({
  ...
  // user-centric). For each entity X compute the strongest path from
  ...
  const drawLink = useCallback(
    (link: ForceLink, ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      // the user and the entity, rather than a state chip with two edges.
      ...
      {intersectionEntityIds && (
        <>
          <button
            ...
            title="0-hop: State/Event nodes connected directly to BOTH anchors (Marriage, Family Time, Parent of Sam, ...)."
          >
