...
def write_proposals_for_window(
    session,
    window_id: str,
    nodes: List[Dict[str, Any]],
    edges: List[Dict[str, Any]],
    *,
    ...
    """Persist legacy chat-window extractor output as claim_proposal groups.

    Used by the now-archived kg_chat_pipeline_parallel path; still called by
    backfill / replay tooling that operates on kg_chat_conversation_window
    rows. New code should produce a (window_id, enrichment_id) anchor and
    call ``write_one_proposal_group`` directly.

    Returns a stats dict: ``{groups_written, nodes_written, edges_written,
    evidence_written, skipped_already_written, groups_skipped_placeholder,
    enriched}``.
    """
    stats = {
        "groups_written": 0,
    ...
    if _already_written_for_window(session, window_id):
        stats["skipped_already_written"] = 1
        return stats

    anchor = _fetch_window_anchor(session, window_id)

    # Enrich once for the whole window's node set. Extractor emits bare
    # structure; meta_data_add adds dates, aliases, confidence, etc.
    nodes = _enrich_nodes_via_meta_data_add(nodes, edges, anchor, stats)

    for component_nodes, component_edges in split_into_connected_components(nodes, edges):
        if not component_nodes and not component_edges:
            continue
        write_one_proposal_group(
            session,
            nodes=component_nodes,
            edges=component_edges,
            anchor=anchor,
            window_id=window_id,
            projection_id=anchor.get("projection_id"),
            extractor_agent_name=extractor_agent_name,
            extractor_model=extractor_model,
            extractor_prompt_version=extractor_prompt_version,
            extraction_run_id=extraction_run_id,
