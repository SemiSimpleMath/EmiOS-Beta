...
class Node(Base):
    ...
    # Identity sentence (2026-06-12): the minimal definite description that
    # uniquely picks out this referent in THIS graph ("Jouko and Susie's
    # house in Marysville") — a PROJECTION derived from label + anchor
    # edges + era dates, regenerated by the nightly refresh when
    # identity_inputs_hash drifts. NOT the original_sentence (that is the
    # first observation, an accident of arrival order). Embedded in the
    # node_identity_embeddings chroma collection; resolution and merge
    # judges read it. See scratch/IDENTITY_SENTENCE_SPEC.md.
    identity_sentence = Column(Text, nullable=True)
    identity_inputs_hash = Column(String, nullable=True)
