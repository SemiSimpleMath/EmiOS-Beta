"""LLM-rated edge importance for the KG.

Iterates over every KG edge in batches and asks `me::edge_importance_rater`
(retained name for now) to score it 0-10 from the source entity's
perspective. Scores are written to the `kg_edge_metadata.importance` column
directly — that column was reserved for this purpose and historically held
NULL for every edge.

Originally built for the /me lens (hence the agent namespace), but the
output feeds card-tag importance gating, wiki inclusion, and PageRank
weighting too. Relocated from app/me/ to app/assistant/kg/ 2026-05-11.
...
"Jukka ↔ Phil's dad" (met-once acquaintance). Entity card tagging uses
edge importance to derive State/Event/Goal importance via the formula
``max(src_imp × edge_imp / 10)`` across incoming edges.

Usage:
    python -m app.assistant.kg.edge_importance     # rate every edge, persist to DB
"""
