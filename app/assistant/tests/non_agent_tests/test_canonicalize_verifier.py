...
def _run(beliefs, vecs=_VECS, focus_keys=None):
    store = _FakeStore(beliefs, vecs)
    merges = C._run_verifier_dedup_pass(
        beliefs, "test", store, _FakeFactory(), scope_context=None, focus_keys=focus_keys,
    )
    return store, merges


def test_full_pass_merges_dups_and_keeps_lookalikes_apart():
    beliefs = _make_beliefs()
    store, merges = _run(beliefs)
...
def test_focus_keys_restricts_proposals_to_new_belief():
    ...
    beliefs = _make_beliefs()
    store, merges = _run(beliefs, focus_keys={"food.salmiakki.b"})
...
def test_keyword_channel_catches_embedding_miss():
    ...
    store, merges = _run(beliefs, _KW_VECS)
    assert merges == 1                                          # carbonara pair merged via keyword
