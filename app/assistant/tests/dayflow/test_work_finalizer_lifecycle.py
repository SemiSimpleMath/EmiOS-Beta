...
def _step(store, wid, nid, *statuses):
    for st in statuses:
        store.apply("set_status", {"work_id": wid, "node_id": nid, "status": st}, work_id=wid)
...
def test_resolve_close_completes_wo_with_open_sibling(store):
    ...
    _step(store, wid, a, "actionable", "dispatched", "done", "closed")
    store.apply("set_work_status", {"work_id": wid, "status": "done"}, work_id=wid)
    wo = store.load(wid)
...
def test_resolve_abandon(store):
    ...
    store.apply("set_work_status", {"work_id": wid, "status": "abandoned"}, work_id=wid)
    wo = store.load(wid)
