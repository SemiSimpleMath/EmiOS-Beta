"""CLI to run the romantic_proposer once.

Usage:
    .venv/Scripts/python.exe -m app.assistant.subconscious.run_romantic_proposer
    .venv/Scripts/python.exe -m app.assistant.subconscious.run_romantic_proposer --dry-run
    .venv/Scripts/python.exe -m app.assistant.subconscious.run_romantic_proposer --no-persist

Daily relationship brain. Reads relationship signals + concerns + key
dates; proposes small calibrated date nights / gestures / anniversary
prep / quality time for the next 7-14 days. Mints intention.romantic
pods.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run the romantic_proposer once.")
    ...
    print("\n[2/4] Invoking romantic_proposer...")
    agent = DI.agent_factory.create_agent("romantic_proposer")
    if agent is None:
        print("ERROR: failed to create romantic_proposer agent.")
        sys.exit(1)
    scope = load_scope_for_source(
        kind="subsystem",
        source_id="subconscious",
        actor_id="run_romantic_proposer",
        identity_overrides={"owner_id": "system", "scope_id": "subconscious::romantic_proposer", "surface": "internal"},
    )
    ...
    try:
        ...
        logger.exception("romantic_proposer agent raised")
        sys.exit(2)
    ...
    for p in proposals:
        ...
        cost = p.get("cost_estimate_usd")
        if cost is not None:
            print(f"          cost: ${cost}")
        if p.get("requires_babysitter"):
    ...
    summary = apply_romantic_proposer_output(output)
