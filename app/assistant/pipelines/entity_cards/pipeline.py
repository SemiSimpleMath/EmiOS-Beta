...
from .steps import GenerateEntityCardsStep, PageRankStep, PruneEntityCardsDryRunStep

logger = get_logger(__name__)


class EntityCardsPipeline:
    pipeline_id = "entity_cards"

    def __init__(self) -> None:
        self._scope_policy = load_pipeline_scope_policy(self.pipeline_id)
        # Default: generate missing cards, then produce a pruning dry-run report.
        self._runner = PipelineRunner(
            steps=[
                PageRankStep(),
                GenerateEntityCardsStep(),
                PruneEntityCardsDryRunStep(),
            ]
        )
