...
class CriticPreNode(ControlNode):
    ...
    def _required_str(cfg: dict, key: str) -> str:
        ...
        if not isinstance(raw, str) or not raw.strip():
            raise ValueError(f"critic pre requires non-empty '{key}'.")
        return raw.strip()
    ...
    def _optional_str(cfg: dict, key: str, default: str) -> str:
        ...
        if not isinstance(raw, str) or not raw.strip():
            raise ValueError(f"critic pre key '{key}' must be non-empty when provided.")
        return raw.strip()
    ...
    def _optional_node(cfg: dict, key: str) -> str | None:
        ...
        if raw is None:
            return None
        if not isinstance(raw, str) or not raw.strip():
            raise ValueError(f"critic pre key '{key}' must be a non-empty string when provided.")
        return raw.strip()
