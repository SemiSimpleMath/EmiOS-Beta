...
class WebNavigateBackSnapshot(BaseTool):
    SERVER_ID = "npm/playwright-mcp"
    MCP_BACK = "browser_navigate_back"
    MCP_SNAPSHOT = "browser_snapshot"

    def __init__(self):
        super().__init__("web_navigate_back_snapshot")

    def execute(self, tool_message: ToolMessage) -> ToolResult:
        _args = (tool_message.tool_data or {}).get("arguments", {}) or {}

        try:
            server_entry = DI.tool_registry.get_mcp_server_entry(self.SERVER_ID)
        except Exception:
            logger.debug("web_navigate_back_snapshot failed to resolve MCP server", exc_info=True)
            server_entry = None
        if not isinstance(server_entry, dict):
            return make_tool_error(
                error_code="mcp_server_missing",
                message=f"web_navigate_back_snapshot error: MCP server entry missing for {self.SERVER_ID}",
                abort_policy="abort_tool",
                retryable=False,
                details={"server_id": self.SERVER_ID},
            )

        back_resp = mcp_stdio_call_tool(
            server_entry=server_entry,
            tool_name=self.MCP_BACK,
            arguments={},
        ...
        if back_error:
            return make_tool_error(
                ...
                abort_policy="abort_tool",
                retryable=True,
                details={"backend": "mcp", "server_id": self.SERVER_ID, "mcp_tool_name": self.MCP_BACK},
            )
        ...
        if snap_error:
            return make_tool_error(
                ...
                message=f"web_navigate_back_snapshot error: browser_snapshot failed after navigate_back: {snap_text}",
                abort_policy="abort_tool",
                retryable=True,
        ...
        content = "web_navigate_back_snapshot:\n- status: navigated_back"
        content = f"{content}\n- snapshot_id: {snapshot_id}"
        content = f"{content}\n- actionable_elements: {len(elements)}"
        return ToolResult(
            result_type="web_navigate_back_snapshot",
            content=content,
            data={
                "ok": True,
                "snapshot_id": snapshot_id,
...
def get_tool_class():
    return WebNavigateBackSnapshot
